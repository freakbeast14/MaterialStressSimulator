...
def _run_job(job_id: str, payload: SimulationRequest) -> None:
    status = _jobs[job_id]
    try:
        _update_progress(status, 10, 25, 0.2)
        _update_progress(status, 25, 55, 0.25)
        _update_progress(status, 55, 80, 0.2)

        results = _solve_with_fenics(job_id, payload)
        status.status = "completed"
        status.progress = 100
        status.results = results
    except Exception as exc:
        fallback = _build_stub_results(job_id, payload)
...
def _solve_with_fenics(job_id: str, payload: SimulationRequest) -> Dict[str, float | int | list | str]:
    import numpy as np
    import dolfin as df
    ...
    time_series = _build_time_series(job_id, payload, max_stress)
...
def _build_stub_results(job_id: str, payload: SimulationRequest) -> Dict[str, float | int | list | str]:
